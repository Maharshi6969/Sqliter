...
//frees the input buffer memory to prevent leaks
void close_input_buffer(InputBuffer* input_buffer){
    free(input_buffer->buffer);
    free(input_buffer);
}

//mets commands to control the flow eg.. exit(to quit the program)
MetaCommandResult do_meta_command(InputBuffer* input_buffer, Table* table){
...
Pager* pager_open(const char* filename){
    ...
    if(file_length%PAGE_SIZE!=0){
        printf("Corrupt file!! not a whole number");
        exit(EXIT_FAILURE);
    }
...
uint32_t* leaf_node_num_cells(void* node){
    return (uint32_t*)((char*)node+LEAF_NODE_NUM_CELL_OFFSET);
}
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value){
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
    ...
    //evenly between old and new nodes
    //starting from left, move each key to correct position

    if (LEAF_NODE_MAX_CELLS == 0) {
        printf("Leaf node split attempted on empty leaf.\n");
        exit(EXIT_FAILURE);
    }
...
void create_new_root(Table* table, uint32_t right_child_page_num){
    ...
    set_node_type(left_child, get_node_type(root));
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num){
    ...
    //If we're already at the max number of cells for a node, we cannot increment
    //before splitting. Incrementing without inserting a new key/child pair
    //and immediately calling internal_node_split_and_insert has the effect
    //of creating a new key at (max_cells + 1) with an un-init value
